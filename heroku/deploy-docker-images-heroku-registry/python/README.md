...
```
heroku create -a pythondockerdemo
Creating pythondockerdemo... done, stack is heroku-18
https://pythondockerdemo.herokuapp.com/ | https://git.heroku.com/pythondockerdemo.git
```
If needed set Heroku git to your app
```
heroku git:remote -a pythondockerdemo
```

```
##Heroku will build the image when doing heroku container:push:
##docker build -t pythondockerdemo:1.0 . This step is not necessary
```

Inside the repo folder type: 
```
git init and heroku git:remote -a pythondockerapp #Your App Name on Heroku

heroku container:push web -a pythondockerdemo
```

Then release the image to your app:

```
heroku container:release web -a pythondockerdemo
```
...
```
heroku open
heroku open pythondockerdemo (My App name on Heroku)
```
